...
{
   void setter();
   double findCountribution(int no_developers,double revenue,int pos);
}
...
{   
    ...
    public boolean isSufficentBalance(double v)
    {
        if(v<balance)
            return true;
        else
...
{   
   ...
   {
       int i=pos;i++;
       System.out.print("Enter person "+i+" Countribution (%): ");
       double v=kb.nextInt();
        ...
        while(true)
        {
            ...
            switch(ch)
            {
                ...
                case 3:
                   ...
                   String namm=kb.nextLine();
                   for(int j=0;j<account.size();j++)
                     {
                       if(namm.equals(account.get(j).getName()))
                       {
